...
def on_cancel(doc, method):
    ...
    if getattr(doc, "journal_entry_ref", None):
        try:
            je = frappe.get_doc("Journal Entry", doc.journal_entry_ref)
            if je.docstatus == 1:
                je.cancel()
        except Exception as e:
            frappe.log_error(f"Error cancelling linked Journal Entry: {str(e)}")
    
    # Clear reference in Share Agreement if linked
    if getattr(doc, "source_document_type", None) == "Share Agreement" and getattr(doc, "source_document_name", None):
        if frappe.db.exists("Share Agreement", doc.source_document_name):
            # set fields individually to avoid passing a dict to set_value
            frappe.db.set_value("Share Agreement", doc.source_document_name, "share_movement_ref", None, update_modified=False)
            frappe.db.set_value("Share Agreement", doc.source_document_name, "status", "Draft", update_modified=False)
    
    # Reset status on the movement
    try:
        doc.db_set("status", "Cancelled", update_modified=False)
    except Exception:
        # fallback if doc.db_set is not available
        frappe.db.set_value(doc.doctype, doc.name, "status", "Cancelled", update_modified=False)
...
def on_trash(doc, method):
    ...
    if getattr(doc, "docstatus", 0) == 2:  # Cancelled
        if getattr(doc, "journal_entry_ref", None):
            try:
                if frappe.db.exists("Journal Entry", doc.journal_entry_ref):
                    je = frappe.get_doc("Journal Entry", doc.journal_entry_ref)
                    if je.docstatus == 2:
